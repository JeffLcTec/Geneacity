    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = event.pos
                for rect, persona in rects:
                    if rect.collidepoint(mouse_pos):
                        handle_character_click(persona)
...
def arbol_persona(persona):
    ...
    if persona["father"] and persona["mother"]:
        ...
        familiares=[three_persona.agregar_nodo(Nodo(padre["id"], padre["name"])),three_persona.agregar_nodo(Nodo(madre["id"], madre["name"]))]
...
def ver_familia(three,puntaje,Ver):
    ...
    if Ver==True:
        ...
        canvas.create_text(screen_width // 2, 600, text=f"Puntaje: {puntaje}", font=("Arial", 36, "bold"), fill="Blue")
        ventana.mainloop()
...
def reproduccion(jugador,nombre,root,three):
        ...
        three.agregar_nodo(Nodo(child["id"], child["name"]))
...
def game(jugador):
    ...
    from utilidades import current_frame,frame_count,frame_actual,frames,ventana_parentezco,puntaje_actual, j_info_open,jsons, habitantes, contadores, sprites, size, screen, image, image_rect, casa_img, x, y, jugadorx, jugadory, clock, speed, fondo, tile_size, tiles, window_open, persona_info_open, esc_press, lockW, lockA, lockS, lockD, cont_casas
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                if j_info_open:
                    if arbol.collidepoint(event.pos):
                        ver_familia(three,puntaje_actual,Ver=True) 
